...
func (fe *frontendServer) userlookupresponse(w http.ResponseWriter, r *http.Request) {
	...
	span, bad := tracer.SpanFromContext(r.Context())
	spanctx := span.Context()
	traceid := tracer.TraceIDHex(spanctx)
	spanid := tracer.SpanIDHex(spanctx)
	traceparent := "00-" + traceid + "-" + spanid + "-00"
	client := http.Client{Transport: otelhttp.NewTransport(http.DefaultTransport)}
	httprequest, err := http.NewRequestWithContext(r.Context(), "GET", targetid, nil)
	httprequest.Header.Add("traceparent", traceid)
	if err != nil {
		fmt.Fprint(w, err)
	}
	...
	message := string(body)
	request := formatRequest(httprequest)

	if err := templates.ExecuteTemplate(w, "error", map[string]interface{}{
		"session_id":      sessionID(r),
		"request_id":      r.Context().Value(ctxKeyRequestID{}),
		"error":           message + " : " + request + ":" + traceparent + strconv.FormatBool(bad),
		"status_code":     400,
		"rum_realm":       os.Getenv("RUM_REALM"),
		"rum_auth":        os.Getenv("RUM_AUTH"),
		"rum_app_name":    os.Getenv("RUM_APP_NAME"),
...
//boobo0b00aaaaaaaaaaaaaaadsdfsdfsdfsdfsdfasdasdasdsdfsdfsdffsdsd
